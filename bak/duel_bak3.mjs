...
- 最後は必ず質問で終える
- 箇条書き・コード・URLは禁止
- 文頭の言い回しは毎回変える。「はい」「あっ」「えっと」「なるほど」などの定型的な出だしを連続で使わない
- 同じ文頭になりそうな場合は、前置きを置かずに本題から話し始める
- 文頭は状況に応じて自然に変化させ、毎回少し違う表現を選ぶ
- 語尾が毎回「だよね」「かな」「かも」ばかりにならないように、言い切り/言い換え/疑問形を混ぜる
`;
}
...
async function generate(speaker, history, input) {
  ...
  out = normalizeEmotionTagged(out, "neutral");
...
function buildRecentTranscript(turnLog) {
  ...
  return lastN.map((t) => `${t.who}: ${oneLine(t.text)}`).join("\n");
}
...
async function runConversation(sessionNo) {
  ...
  for (let i = 1; i <= TURNS; i++) {
    ...
    if (i % TOPIC_INTERVAL === 0) {
      ...
      // ★ 話題切替の台詞（ここも微調整ポイント）
      last = `[neutral]話変わるけどいい？${topic}ってどう？`;
