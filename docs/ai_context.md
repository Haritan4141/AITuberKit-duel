...
| 2026-05-12 | Claude Opus 4.7 | 初回 push 実施 (`a5db118..a333d0e`)。GitHub の 100MB 制限で push が拒否されたため、**ユーザー明示許可のもと** `git filter-branch` で履歴の VRM (`aituber-kit/public/vrm/Mafuyu_VRM.vrm` / `MANUKA.vrm`、および旧 `aituber-kit-B/` 配下) を全コミットから除去 → `refs/original/` 削除 + `git gc --prune=now --aggressive` でオブジェクトをクリーン → `git push --force-with-lease origin main` でリモートに反映。今後 VRM は `.gitignore` で除外。**今後 git filter-branch / filter-repo / force push が必要な状況になった場合も、ai_context.md のルール通り必ずユーザーに明示許可を取ること**。退避用に `backup/before-vrm-filter` ローカルブランチを作成したが、`--all` で filter-branch を回したため backup 自体も書き換えられている点に注意（元状態は reflog 経由でしか復元不可、reflog は expire 済み）。 |
